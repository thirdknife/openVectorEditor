...
test('changes the caret position from its initial value', function(t) {
    t.notEqual(55, tree.get('caretPosition'));
    setCaretPosition(55);
    t.equal(55, tree.get('caretPosition'));
    setCaretPosition(59);
    t.equal(59, tree.get('caretPosition'));
    t.end();
});
test('changes the caret position to -1 if passed anything but a non-negative integer', function(t) {
    t.notEqual(-1, tree.get('caretPosition'));
    setCaretPosition(false);
    t.equal(-1, tree.get('caretPosition'));
    t.end();
});